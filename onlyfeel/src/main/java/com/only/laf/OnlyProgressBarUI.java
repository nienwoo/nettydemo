...
import com.only.OnlyProgressBar;
import com.only.box.OnlyImageBox;
import com.only.box.UIBox;
import com.only.util.OnlyUIUtil;

public class OnlyProgressBarUI extends BasicProgressBarUI {
	...
	private static final Image BG_IMAGE_H = OnlyImageBox.getClassPathImageByName("progress_bar_background_h.png");
	private static final Image BG_IMAGE_V = OnlyImageBox.getClassPathImageByName("progress_bar_background_v.png");

	private static final Image IMAGE_H = OnlyImageBox.getClassPathImageByName("progress_bar_h.png");
	private static final Image IMAGE_V = OnlyImageBox.getClassPathImageByName("progress_bar_v.png");

	public static ComponentUI createUI(JComponent c) {
		return new OnlyProgressBarUI();
	}
	...
	protected void paintDeterminate(Graphics g, JComponent c) {
		...
		paintBackground(g2d, c, insets, barRectWidth, barRectHeight);

		if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
			Rectangle paintRect = new Rectangle(insets.left, insets.top, amountFull, barRectHeight);
			OnlyUIUtil.paintImage(g, IMAGE_H, insets, paintRect, c);
			// createColors(progressBar.getForeground());
			// paint = new LinearGradientPaint(0, insets.top, 0, barRectHeight +
			// insets.top, FRACTIONS, colors);
			// g2d.setPaint(paint);
			// g2d.fillRoundRect(insets.left, insets.top, amountFull,
			// barRectHeight, barRectHeight, barRectHeight);
		} else {
			Rectangle paintRect = new Rectangle(insets.left, insets.top, barRectWidth, amountFull);
			OnlyUIUtil.paintImage(g, IMAGE_V, insets, paintRect, c);
			//
			// createColors(progressBar.getForeground());
			// paint = new LinearGradientPaint(barRectWidth + insets.left, 0,
			// insets.left, 0, FRACTIONS, colors);
			// g2d.setPaint(paint);
			// g2d.fillRoundRect(insets.left, insets.top + (barRectHeight -
			// amountFull), barRectWidth, amountFull, barRectWidth,
			// barRectWidth);
		}
	...
	protected void paintIndeterminate(Graphics g, JComponent c) {
		...
		if (boxRect != null) {
			if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
				Rectangle paintRect = new Rectangle(boxRect.x, boxRect.y, boxRect.width, boxRect.height);
				OnlyUIUtil.paintImage(g, IMAGE_H, insets, paintRect, c);

				// createColors(progressBar.getForeground());
				// paint = new LinearGradientPaint(0, insets.top, 0,
				// barRectHeight + insets.top, FRACTIONS, colors);
				// g2d.setPaint(paint);
				// g2d.fillRoundRect(boxRect.x, boxRect.y, boxRect.width,
				// boxRect.height, barRectHeight, barRectHeight);
			} else {
				Rectangle paintRect = new Rectangle(boxRect.x, boxRect.y, boxRect.width, boxRect.height);
				OnlyUIUtil.paintImage(g, IMAGE_V, insets, paintRect, c);

				// createColors(progressBar.getForeground());
				// paint = new LinearGradientPaint(barRectWidth + insets.left,
				// 0, insets.left, 0, FRACTIONS, colors);
				// g2d.setPaint(paint);
				// g2d.fillRoundRect(boxRect.x, boxRect.y, boxRect.width,
				// boxRect.height, barRectWidth, barRectWidth);
			}
		}
	...
	private void paintBackground(Graphics2D g2d, JComponent c, Insets insets, int barRectWidth, int barRectHeight) {
		if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {

			Rectangle paintRect = new Rectangle(insets.left, insets.top, barRectWidth, barRectHeight);
			OnlyUIUtil.paintImage(g2d, BG_IMAGE_H, insets, paintRect, c);
			...
			Rectangle paintRect = new Rectangle(insets.left, insets.top, barRectWidth, barRectHeight);
			OnlyUIUtil.paintImage(g2d, BG_IMAGE_V, insets, paintRect, c);
			//
			// createColors(progressBar.getBackground());
			// paint = new LinearGradientPaint(barRectWidth + insets.left, 0,
			// insets.left, 0, FRACTIONS, colors);
	...
	private void paintString(Graphics g, int x, int y, int width, int height, int fillStart, int amountFull, Insets b) {
		...
		boolean is3D = progressBar instanceof OnlyProgressBar;
		Color fontColor = is3D ? ((OnlyProgressBar) progressBar).getFontColor() : Color.ORANGE;
		Color fontCoverClor = is3D ? ((OnlyProgressBar) progressBar).getFontCoverClor() : UIBox.getWhiteColor();
